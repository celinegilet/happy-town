...
class AttribuerCadeauxTest {
    ...
    static {
        CadeauxByTrancheAgeFileAdapter cadeauxByTrancheAgeFileProvider = new CadeauxByTrancheAgeFileAdapter();
        try {
    ...
    void attribuerCadeaux_habitantTrancheAge0_3() {
        ...
        doReturn(newArrayList(habitant)).when(habitantPort).getEligiblesCadeaux(NOW_MINUS_ONE_YEAR);
    ...
    private void verifyNotificationsSent(Habitant habitant, String refCadeau) {
        ...
        verify(notificationPort, times(2)).notifier(toCaptor.capture(), subjectCaptor.capture(), templatePathCaptor.capture(), templateArgsCaptor.capture());
